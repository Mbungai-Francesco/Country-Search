...
function Home() {
    const [ countries, setCountries ] = useState<IAllDataItem[]>()
	const [inCountries, setinCountries] = useState<IAllDataItem[]>();
	...
	useEffect(() => {
		fetchAllData().then((data) => {
			if (data) {
                setinCountries(data.slice(0, 50));
                setCountries(data);
            }
		});
	}, []);
	...
	const searchBarToggle = () => {
		if (searchBar === "hidden")
			setSearchBar(
				classer(
					"absolute w-full py-2 px-4 bg-white dark:bg-myBlue-dark mt-2 z-10"
				)
			);
		...
		if (countries) {
			...
			setinCountries(() =>
				countries.filter((country) =>
					country.name.common.toLocaleLowerCase().startsWith(val)
				)
			);
		}
	};
		...
		<>
			<div className={classer("mb-8 justify-between sm:flex")}>
				<label
					htmlFor=""
					className={classer(
						"mb-4 flex py-2 px-4 items-start bg-white dark:bg-myBlue-dark sm:mb-0"
					)}
				>
					<svg
						...
						className={classer(
							"feather feather-search text-gray-500 black:text-white"
						)}
					>
					...
					<input
						className={classer(
							"ml-2 bg-transparent w-80 outline-none text-gray-500 black:text-gray-400 text-xs font-medium black:font-normal"
						)}
						ref={inputRef}
				...
				<div className={classer("relative text-gray-500")}>
					<div
						className={classer(
							"flex justify-between py-2 px-4 items-start bg-white dark:bg-myBlue-dark cursor-pointer"
						)}
			...
			>
				{inCountries ? (
					inCountries?.map((country) => {
						const countryId =
							String(country.latlng[0]) + String(country.latlng[1]);
						return (
							<Link to={`/:${countryId}`} key={countryId}>
